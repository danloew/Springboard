...
SELECT COUNT( facid ) , COUNT( name ) , membercost
FROM Facilities
WHERE membercost =0
...
SELECT sub.facility, CONCAT( sub.firstname, ' ', sub.surname ) AS name
FROM (
...
FROM (
	...
	SELECT sub.facility AS facility, SUM(sub.cost) AS total_revenue
